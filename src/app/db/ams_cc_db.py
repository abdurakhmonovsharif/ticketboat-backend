...
async def create_credit_card(card_data: CreditCardCreateRequest):
    try:
        ...
        values = {
            ...
            "type": card_data.type,
            "company_id": card_data.company_id,
...
async def get_credit_cards_by_filters(
    ...
    try:
        ...
        if company_ids and len(company_ids) > 0:
            ...
            filters.append(f"(COALESCE(a.company_id, cc.company_id) IN ({placeholders}))")
        ...
        if search_query:
            filters.append("""
                (
                    ...
                    COALESCE(account_company.name, card_company.name) ILIKE :search OR
        ...
        query = f"""
            SELECT
                ...
                COALESCE(account_company.name, card_company.name) as "company",
                cc.status as "status",
            ...
            LEFT JOIN ams.company card_company ON cc.company_id::TEXT = card_company.id::TEXT
...
async def get_credit_card_for_edit(card_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        result['company'] = json.loads(result['company']) if result['company'] else None
