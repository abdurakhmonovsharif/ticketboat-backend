...
from app.database import get_pg_open_distribution_readonly_database, get_pg_open_distribution_database
    ...
    try:
        query = """
            ...
            show_markup AS (
                SELECT 
                    osc.outbox_show_id,
                    ROUND(AVG(osc.markup_percent), 2) as average_markup
                FROM open_dist.outbox_sync_config osc
                WHERE osc.markup_percent IS NOT NULL
            ...
            show_listings AS (
                ...
                LEFT JOIN open_dist.outbox_current_listing ocl ON ocl.event_id = oe.id 
                    AND ocl.updated_at > CURRENT_TIMESTAMP - interval '7 minutes'
                GROUP BY oe.show_id
            ),
            active_listings AS (
                SELECT 
                ...
                LEFT JOIN open_dist.trade_desk_current_listing tcl ON tcl.event_id = osc.trade_desk_event_id::TEXT
                    AND tcl.updated_at > CURRENT_TIMESTAMP - interval '7 minutes'
            ...
            show_synced_events AS (
                SELECT 
                    osc.outbox_show_id,
                    COUNT(CASE WHEN osc.sync_active = true THEN 1 END) as synced_events_count
                FROM open_dist.outbox_sync_config osc
                LEFT JOIN open_dist.outbox_event oe ON osc.outbox_event_id = oe.id
            ...
            show_mapping AS (
                SELECT 
                    ...
                    COUNT(osc.trade_desk_event_id) as mapped_events
                FROM open_dist.outbox_event oe
                LEFT JOIN open_dist.outbox_sync_config osc ON osc.outbox_event_id = oe.id
                WHERE oe.updated_at > current_timestamp - interval '24 hours'
    ...
    try:
        query = """
            ...
            SELECT 
                ...
                -- Add active mark_override rule details
                (
                    ...
                    WHERE lro.is_active = true
                      ...
                      AND (
                          ...
                          OR
                          -- Timing: hours_before_event (check if within window)
...
async def update_sync_active_by_show_id(show_id: str, sync_active: bool):
    """Update sync_active for all events in a show."""
    try:
        query = """
...
async def get_unmapped_events() -> list[UnmappedEventModel]:
    try:
        query = """
            ...
            AND event_start > current_timestamp + interval '6 hours'
            AND osc.outbox_event_id IS NULL
            ORDER BY oe.event_start ASC
        """
        result = await get_pg_open_distribution_readonly_database().fetch_all(query=query)
        unmapped_events = []
    ...
    try:
        ...
        query = """
            INSERT INTO open_dist.outbox_sync_config (
                ...
                uuid_generate_v4(), :trade_desk_event_id, :outbox_event_id, false, 10.00, :event_date, :outbox_show_id, :trade_desk_event_name, :event_timezone
            ) RETURNING *;
        """
        values = {
