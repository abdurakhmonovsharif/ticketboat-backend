...
from app.model.ams_models import (
    ...
    EmailTwoFAResponseModel, EmailCommonFieldsRequest,
)
...
async def update_account_automators(account_id: str, automator_ids: list[str]):
    try:
        db = get_pg_database()
        async with db.transaction():
...
async def update_account_point_of_sale(account_id: str, pos_ids: list[str]):
    try:
        db = get_pg_database()
        async with db.transaction():
            delete_query = """
        ...
        for mapping in persona_mappings:
            ...
            try:
                ...
                async with ts_service:
                    ...
                    persona_data = await ts_service.get_persona(persona_id=persona_id)
                    if not persona_data:
                    ...
                    await ts_service.update_persona(
                        persona_id=persona_id,
...
async def _update_persona_proxy(email_request: list[dict], proxy_id: str, automator_id: str, automator_name: Optional[str] = None):
    try:
        ...
        async with ts_service:
            persona_data = await ts_service.get_persona(email=str(email_request))
                ...
                if proxy_str:
                    ...
                    if proxy_model:
                        for pid in persona_ids:
                            try:
                                resp = await ts_service.update_persona_proxy(persona_id=str(pid), proxy_payload=proxy_model)
