...
class CreateTicketSuitePersonasRequest(BaseModel):
    ...
    primary_ids: Optional[List[str]] = Field(
        default=None,
        description="List of specific primary IDs to create personas for. Required if all_primaries is False."
    )
    all_primaries: bool = Field(
        default=False,
        description="If True, creates personas for all available primaries that pass validation. "
                    "If False, only creates personas for the specified primary_ids."
    )
    
    @field_validator("primary_ids")
    def validate_primary_ids(cls, v, info):
        """Ensure primary_ids is provided when all_primaries is False"""
        all_primaries = info.data.get("all_primaries", False)
        if not all_primaries and not v:
            raise ValueError("primary_ids is required when all_primaries is False")
        return v
