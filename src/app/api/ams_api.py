...
from app.service.ticketsuite.persona_creation_service import get_persona_creation_service
    ...
    try:
        ...
        for automator in automators:
            ...
            async with ts_service:
                exists = await ts_service.get_persona(email=email)
...
async def handle_create_ticketsuite_personas_for_accounts(
        payload: CreateTicketSuitePersonasRequest,
        user: User = Depends(get_current_user_with_roles(["ams_admin", "ams_accounts_editor"])),
):
    """
    Create TicketSuite personas for account-primary combinations.
    Validates that each account has both company and automator fields filled out.
    
    Args:
        payload: CreateTicketSuitePersonasRequest with:
            - account_ids: List[str] - Account IDs to create personas for (required)
            - primary_ids: Optional[List[str]] - Specific primary IDs to create personas for
              (required when all_primaries=False)
            - all_primaries: bool - Flag to control primary selection (default: False)
              * If True: Creates personas for ALL available primaries that pass validation
              * If False: Creates personas ONLY for the specified primary_ids
    
    Returns:
        List[dict]: List of TicketSuiteSyncResponseItem with:
            - account_id: str
            - email_address: str
            - sync_results: List of TicketSuiteSyncResult with:
                - primary_name: str | None
                - status: "success" | "error"
                - status_code: int | None
                - response: unknown (success message)
                - error: str (error message)
    
    Examples:
        # Create personas for specific primaries only
        {
            "account_ids": ["acc-uuid-1", "acc-uuid-2"],
            "primary_ids": ["prim-uuid-1", "prim-uuid-2"],
            "all_primaries": false
        }
        
        # Create personas for all available primaries
        {
            "account_ids": ["acc-uuid-1", "acc-uuid-2"],
            "all_primaries": true
        }
    """
    try:
        service = get_persona_creation_service()
        return await service.create_personas_for_accounts(
            account_ids=payload.account_ids,
            primary_ids=payload.primary_ids,
            all_primaries=payload.all_primaries
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Internal Server Error: {str(e)}")
